...
class MycobotTopics(object):
    ...
    def start(self):
        ...
        ps = threading.Thread(target=self.pub_state)
    ...
    def pub_state(self):
        """publish both real coordinates and angles"""
        pub = rospy.Publisher("mycobot/state",
                              MycobotState, queue_size=5)
        ms = MycobotState()
        ...
        while not rospy.is_shutdown():
            ...
            if angles and coords:
                ms.angles.joint_1 = round(angles[0], 2)
                ms.angles.joint_2 = round(angles[1], 2)
                ms.angles.joint_3 = round(angles[2], 2)
                ms.angles.joint_4 = round(angles[3], 2)
                ms.angles.joint_5 = round(angles[4], 2)
                ms.angles.joint_6 = round(angles[5], 2)

                ms.coords.x = round(coords[0])
                ms.coords.y = round(coords[1])
                ms.coords.z = round(coords[2])
                ms.coords.rx = round(coords[3])
                ms.coords.ry = round(coords[4])
                ms.coords.rz = round(coords[5])
